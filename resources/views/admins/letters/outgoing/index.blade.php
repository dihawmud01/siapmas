...
@section('content')
    <x-breadcrumb :values="[__('Surat-menyurat'), __('Surat Keluar')]"></x-breadcrumb>
    <div class="card info-card sales-card min-vh-100">
        ...
        <div class="container-fluid p-5">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <h5 class="fw-semibold mb-0">{{ __('Total Surat Keluar: ') }} {{ $totalOutgoing }}</h5>
                <div class="d-flex justify-content-between align-items-end mb-3 flex-wrap gap-3">
                    <form action="{{ url()->current() }}" method="GET" class="d-flex align-items-end flex-wrap gap-2">
                        ...
                        <div class="mb-3">
                            ...
                            <a
                                href="{{ route('dashboard.letters.outgoing.print') . '?' . $query }}"
                                target="_blank"
                    ...
                    <a
                        href="{{ route('dashboard.letters.outgoing.create') }}"
                    ...
                    <table class="table" id="table">
                        <thead>
                            <tr class="fw-bold text-center">
                                <td>{{ __('No.') }}</td>
                                <td class="text-start">{{ __('Nama Surat') }}</td>
                                <td>{{ __('No. Surat') }}</td>
                            ...
                            @foreach ($outgoing as $idx => $letter)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a
                                            href="{{ route('dashboard.letters.outgoing.show', $letter) }}"
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a
                                            href="{{ route('dashboard.letters.outgoing.edit', $letter) }}"
                                            class="btn btn-warning btn-sm me-1"
                                        >
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <form
                                            action="{{ route('dashboard.letters.outgoing.destroy', $letter) }}"
                                            method="POST"
                                            class="d-inline"
                                        >
                                            @csrf
