...
@section('content')
    <div class="card info-card sales-card">
        <div class="container">
            <h2 class="text-center my-2 pt-2">{{ __('Data Anggota') }}</h2>
            <div class="row align-items-center mb-3">
                <div class="col-12 d-flex justify-content-between align-items-end">
                    <div>
                        <h5 class="m-0">{{ __('Total Kader: ') }} {{ $userCounts }}</h5>
                        <div class="d-flex gap-2">

                        <a href="{{ route('users.create') }}"
                           class="btn btn-primary mt-2">{{ __('Tambah Anggota') }}</a>
                        </div>
                    </div>
                ...
                <table class="table" id="table">
                    ...
                    @foreach ($user as $idx => $cadre)
                        <tr>
                            ...
                            <td>{{ $cadre['name'] }}</td>
                            <td>
                                <a href="{{ route('users.pac.list', ['slug' => $cadre->pac->slug]) }}">{{ $cadre->pac->pac }}</a>
                            </td>
                                ...
                                <td class="text-center">
                                    <form action="{{ route('users.destroy', $cadre->id) }}" method="POST">
                                        <a href="{{ route('users.detail', ['id' => $cadre->id]) }}"
                                           class="btn btn-success btn-sm">{{ __('Detail') }}</a>
                                        <a href="{{ route('profile.user', ['slug' => $cadre->slug]) }}"
                                           class="btn btn-secondary btn-sm">{{ __('Profile') }}</a>
                            ...
                            ('{{ __('Apakah Anda yakin ingin menghapus userss ini?') }}')">{{ __('Hapus') }}
                                            </button>
                                    </form>
                                </td>
                            @endif
