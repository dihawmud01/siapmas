...
Route::prefix('mobile')->group(function () {
    ...
    Route::get('/news', [MobileHomeController::class, 'news'])->name('mobilePost');
    Route::get('/news/details/{slug}', [MobileHomeController::class, 'show'])->name('mobilePostShow');
    Route::get('/news/nuOnline/details/{slug}', [MobileHomeController::class, 'nuShow'])->name('nuShow');
    Route::get('/cadres/profile/{slug}', [MobileHomeController::class, 'KaderProfile'])
        ->name('KaderProfile')
        ->middleware(['auth']);
});

Route::middleware(['auth', 'role:1, 2, 3'])->group(function () {
    Route::get('mobile/uploads', [MobileHomeController::class, 'mobileUpload'])
        ->name('mobileUpload')
        ->middleware(['auth']);
    Route::get('mobile/profile', [MobileHomeController::class, 'profile'])
        ->name('profile')
        ->middleware(['auth']);
});
