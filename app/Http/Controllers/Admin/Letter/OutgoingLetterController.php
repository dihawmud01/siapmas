...
class OutgoingLetterController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $outgoing = Letter::outgoing()->agenda($since, $until, $filter);

        if ($user->role_id == 3) {
            $outgoing = $outgoing->where('user_id', $user->id);
        }

        $totalOutgoing = (clone $outgoing)->count();

        $outgoing = $outgoing->render($since, $until, $filter);
